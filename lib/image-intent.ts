/** ChatGPT-style catch-all: verb + optional words + photo/picture/image noun */
const IMAGE_PHRASE =
  /\b(make|create|generate|draw|paint|design|show|give|render|produce|mach(?:e|en|st)?|mache|erstell(?:e|en|st)?|generier(?:e|en|st)?|zeichn(?:e|en|st)?|mal(?:e|en|st)?|yap(?:ay[ıi]m|t[ıi]r|acak)?|olu[şs]tur(?:t[ıu]r|acak|ulur)?|olustur(?:tur|acak)?|[çc]iz(?:eyim|im)?|ciz(?:eyim|im)?|crea(?:r|ndo|do)?|genera(?:r|ndo|do)?|haz(?:er|o)?|fais(?:ons|ez|ait)?|faire|cria(?:r|ndo|do)?|создай|сгенерируй|нарисуй|сделай|اصنع|ارسم|اعمل|أنشئ|انشئ|zeig(?:e|en)?)\b.{0,48}\b(bild(?:er)?|foto(?:s|graf(?:ie)?)?|zeichnung(?:en)?|images?|pictures?|photos?|drawings?|illustrations?|pics?|resim(?:ler|i|imi|ine|de|den)?|g[öo]rsel(?:ler|i|ini|de)?|afbeelding(?:en)?|imagen(?:es)?|immagini?|imagens?|fotograf(?:ía|ia)?|dessins?|obraz(?:ek|u)?|zdj[ęe]ci[ea]?|картин(?:ка|ки|ку)?|изображен(?:ие|ия|ии)?|фото|рисун(?:ок|ка|ки)|صور(?:ة|ه)?|رسم)\b/i;

/** Noun-first: "a photo of …", "Bild von …", "picture of …" */
const IMAGE_NOUN_FIRST =
  /^(?:ein(?:e[mnrs]?|em|er|en)?\s+|a\s+|an\s+|the\s+|bir\s+|bi\s+)?(?:bild(?:er)?|foto(?:s|graf(?:ie)?)?|zeichnung(?:en)?|images?|pictures?|photos?|drawings?|illustrations?|pics?|resim(?:ler|i|imi|ine|de|den)?|g[öo]rsel(?:ler|i|ini|de)?|imagen(?:es)?|immagini?|imagens?|fotograf(?:ía|ia)?|dessins?|obraz(?:ek|u)?|zdj[ęe]ci[ea]?|картин(?:ка|ки|ку)?|изображен(?:ие|ия|ии)?|фото|рисун(?:ок|ка|ки)|صور(?:ة|ه)?|رسم)\s+(?:von|vom|of|of an|of a|of the|de|du|di|con|sobre|i[çc]in|icin)\s+/i;
...
const PROMPT_PREFIXES: RegExp[] = [
  ...
  /^(?:ein(?:e[mnrs]?|em|er|en)?\s+|a\s+|an\s+|the\s+|bir\s+|bi\s+)?(?:bild|foto|photo|picture|image|pic|resim|görsel|gorsel|imagen)\s+(?:von|vom|of|of an|of a|of the|de|du|di|için|icin)\s+(?:ein(?:em|er|en|e)?\s+|a\s+|an\s+|the\s+|bir\s+|bi\s+)?/i
];
...
export function wantsImageGeneration(text: string) {
  ...
  if (IMAGE_PHRASE.test(normalized)) return true;
  if (IMAGE_NOUN_FIRST.test(normalized)) return true;
